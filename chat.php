...
function chatHeartbeat() {
	...
	$sql = "select * from chat where (chat.to = '".mysql_real_escape_string($_GET['me'])."' AND recd = 0 AND DATE(sent) >= DATE_ADD(CURDATE(),INTERVAL -7 DAY) )  order by id ASC ";
...
function openChat() {
	...
	$sql = "select * from chat where (chat.to = '".mysql_real_escape_string($_GET['me'])."' AND chat.from= '".mysql_real_escape_string($_GET['from'])."' AND DATE(sent) >= DATE_ADD(CURDATE(),INTERVAL -7 DAY))   OR (chat.to = '".mysql_real_escape_string($_GET['from'])."' AND chat.from= '".mysql_real_escape_string($_GET['me'])."' AND DATE(sent) >= DATE_ADD(CURDATE(),INTERVAL -7 DAY) ) ";
	$query = mysql_query($sql);
	$items = '';
